...
function renderGraphMap(resolved) {
  ...
  <figcaption class="cc-map-meta">
    ...
    <span class="cc-map-hint">click a node to focus its section · drag to pan · scroll to zoom</span>
  </figcaption>
...
function renderMapSection({ graph = null, sections = [] } = {}) {
  if (graph) {
    return `<section class="cc-map cc-map-graph" aria-label="Concept map">${renderGraphMap(graph)}</section>`;
  }
  const rail = renderTocRail(sections);
  if (!rail) return '';
  return `<section class="cc-map cc-map-toc" aria-label="On this page">${rail}</section>`;
}
...
function renderSearch() {
  ...
  <input type="search" class="cc-search-input" placeholder="Filter sections — press /" aria-label="Filter sections">
</div>`;
}
...
function renderChrome({ uiDefaults = null, sections = [] } = {}) {
  const sectionsJson = safeJson(Array.isArray(sections) ? sections.map((s) => ({ slug: s.slug, text: s.text, level: s.level })) : []);
