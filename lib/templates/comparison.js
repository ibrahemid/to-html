...
function renderOptionCard(opt, idx) {
  ...
  <header class="opt-head">
    <label class="opt-pick" title="Pick this option">
      <input type="radio" name="pick-option" value="${id}" data-option-label="${label}">
...
function render({ markdown, meta, override, buildShell, readAsset }) {
  ...
  const body = `<header class="cmp-head"><h1>${escapeHtml(title)}</h1>${preambleHtml}</header>
...
<aside class="decision-bar" data-decision="comparison">
  <div class="decision-info">
    ...
    <input type="text" id="pick-reason" placeholder="Why? (optional one line)" maxlength="240">
  </div>
