...
function renderTask(task, phaseTitle) {
  ...
  return `<li class="task ${meta.className}" data-task-id="${id}" data-status="${escapeHtml(task.status)}" data-phase="${phaseAttr}">
    <label class="task-focus" title="Focus on this task in the next prompt">
      <input type="checkbox" class="task-focus-input" aria-label="Focus: ${escapeHtml(task.text)}" data-task-id="${id}" data-task-text="${escapeHtml(task.text)}" data-phase-title="${phaseAttr}">
    </label>
    <span class="status-glyph" role="img" aria-label="${escapeHtml(meta.label)}">${meta.glyph}</span>
...
<div class="plan-shell">
  ...
  <div class="plan-main">${phasesHtml}</div>
</div>
</main>
...
function renderFromPlan({ plan, meta, override, buildShell, readAsset, tldrHtml, mapHtml, chromeHtml, uiDefaults }) {
