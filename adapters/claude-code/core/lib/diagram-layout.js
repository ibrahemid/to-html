...
function computeRanks(nodes, edges) {
  ...
  // never hits 0). Propagate ranks forward ONLY from nodes the BFS actually
  // ranked (reachable from a real root), so a pure cycle with no root stays
  // unranked and is rejected downstream. Bounded by node count to terminate.
  let changed = true;
  let guard = 0;
  while (changed && guard++ < nodes.length) {
    ...
    for (const e of edges) {
      ...
      if (!rank.has(e.to) || candidate > rank.get(e.to)) {
