...
function renderSvg(graphLayout, options = {}) {
  ...
  const edgeMarkup = edges.map((edge, idx) => {
    ...
    const labelEl = edge.label
      ...
      : '';
    return `<g class="edge" aria-hidden="true" data-edge-id="e${idx}" data-from="${escapeHtml(edge.fromId)}" data-to="${escapeHtml(edge.toId)}">
  ...
  return `<svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="xMidYMid meet" class="diagram-svg" aria-label="Concept map">
  <defs>
