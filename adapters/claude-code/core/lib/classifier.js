...
const { hasSalvageableGraph } = require('./graph-source');

const OVERRIDE_FENCE = /```to-html[ \t]*\r?\n([\s\S]*?)\r?\n```/;
...
function looksLikeExplainer(signals, markdown) {
  const noCode = markdown.replace(CODE_FENCE, '').replace(INLINE_CODE, '');
  if (/\btl;?dr\b/i.test(noCode)) return true;
...
function shouldRender(signals, markdown, thresholds) {
  ...
  if (typeof markdown === 'string' && hasSalvageableGraph(markdown)) {
    return { render: true, reason: 'graph-salvage' };
  }
...
function classify(markdown) {
  ...
  if (shouldSkip(signals) && !hasSalvageableGraph(stripped)) {
